...
const ThumbnailPicker: React.FC<Props> = ({ preview, onPick }) => {
    ...
    return (
        ...
        >
            ...
            {!src && (
                <>
                    <MdOutlineImage className="text-9xl text-slate-400 mb-8" />
                    <p className="font-sans text-slate-400 text-center">Upload a Thumbnail</p>
                </>
            )}    

            <input ref={refInput} className="hidden" type="file" accept="image/png,image/jpeg" onChange={pick} />
        </div>
    );
};
